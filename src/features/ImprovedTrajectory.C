// -*- C++ -*-  $Id: ImprovedTrajectory.C,v 1.3 2016/12/19 08:15:43 jorma Exp $	
...
namespace picsom {
  static const char *vcid =
    "$Id: ImprovedTrajectory.C,v 1.3 2016/12/19 08:15:43 jorma Exp $";

  static ImprovedTrajectory list_entry(true);

  //===========================================================================

  string ImprovedTrajectory::Version() const {
    return vcid;
  }

  //===========================================================================

  bool ImprovedTrajectory::ProcessOptionsAndRemove(list<string>& l) {
    static const string msg = "ImprovedTrajectory::ProcessOptionsAndRemove() ";
  ...
  int ImprovedTrajectory::printMethodOptions(ostream &str) const {
    /// obs! dunno yet
  ...
  Feature *ImprovedTrajectory::Initialize(const string& img, const string& seg,
    ...
    if (densetrajname!="" && !densetraj.is_open()) {
      densetraj.open(densetrajname, false, bin_data::header::format_float,
		     32*28, 28);
      if (MethodVerbose())
	cout << "ImprovedTrajectory::Initialize() read in densetraj <" << densetrajname
    ...
    if (hogname!="" && !hog.is_open()) {
      hog.open(hogname, false, bin_data::header::format_float,
	       32*96, 96);
      if (MethodVerbose())
	cout << "ImprovedTrajectory::Initialize() read in hog <" << hogname
    ...
    if (hofname!="" && !hof.is_open()) {
      hof.open(hofname, false, bin_data::header::format_float,
	       32*108, 108);
      if (MethodVerbose())
	cout << "ImprovedTrajectory::Initialize() read in hof <" << hofname
    ...
    if (mbhxname!="" && !mbhx.is_open()) {
      mbhx.open(mbhxname, false, bin_data::header::format_float,
		32*96, 96);
      if (MethodVerbose())
	cout << "ImprovedTrajectory::Initialize() read in mbhx <" << mbhxname
    ...
    if (mbhyname!="" && !mbhy.is_open()) {
      mbhy.open(mbhyname, false, bin_data::header::format_float,
		32*96, 96);
      if (MethodVerbose())
	cout << "ImprovedTrajectory::Initialize() read in mbhy <" << mbhyname
  ...
  Feature::featureVector ImprovedTrajectory::CalculateVideoSegment(bool /*print*/) {
    string msg = "ImprovedTrajectory::CalculateVideoSegment() : ";
    if (FileVerbose())
      cout << msg << "videosegment_file=<" << videosegment_file
	   << ">" << endl;

    bool improved_dense = true;

    if (improved_dense)
      return ImprovedDenseTrajectory(videosegment_file);
  ...
  void ImprovedTrajectory::Desc2Vec(std::vector<float>& desc, DescInfo& descInfo,
  ...
  bool ImprovedTrajectory::ProcessRawTraj(const vector<float>& f) {
  ...
  int ImprovedTrajectory::SolveHit(const vector<float>& vin, size_t b, size_t e,
  ...
  bool ImprovedTrajectory::SetHit(int hit, vector<size_t>& hist, size_t dim) {
    if (hit<0)
      return true;
    ...
    if (hit>=(int)hist.size()) {
      cerr << "Index error in ImprovedTrajectory::SetHit()" << endl;
      return false;
    }
  ...
  bool ImprovedTrajectory::AppendHist(vector<float>& v,
  ...
  ImprovedTrajectory::CreateFeatureVector(size_t n) {
    vector<float> v;
    ...
    if (FileVerbose())
      cout << "ImprovedTrajectory::CreateFeatureVector() created " << v.size()
  ...
  Feature::featureVector
  ImprovedTrajectory::ImprovedDenseTrajectory(const string& f) {
    bool print_all = false, print_traj = false;

    if (FileVerbose())
      cout << "ImprovedTrajectory::ProcessRawTraj() " << f << endl;
	...
	improved_trajectory_release::FarnebackPolyExpPyr(prev_grey, prev_poly_pyr, fscales, 7, 1.5);
	...
	continue;
      ...
      improved_trajectory_release::FarnebackPolyExpPyr(grey, poly_pyr, fscales, 7, 1.5);
      improved_trajectory_release::calcOpticalFlowFarneback(prev_poly_pyr, poly_pyr, flow_pyr, 10, 2);
  ...
  bool ImprovedTrajectory::Initialize_pca() {
    // bin_data d(pcaname);
    ...
    // if (MethodVerbose())
    //   cout << "ImprovedTrajectory::Initialize_pca() read in pca <" << gmmname
  ...
  bool ImprovedTrajectory::Initialize_gmm() {
    ...
    // if (MethodVerbose())
    //   cout << "ImprovedTrajectory::Initialize_gmm() read in gmm <" << gmmname << "> dim="
  ...
  bool ImprovedTrajectory::Initialize_kmeans() {
    ...
    // if (MethodVerbose())
    //   cout << "ImprovedTrajectory::Initialize_kmeans() read in kmeans <" << kmeansname
  ...
  bool ImprovedTrajectory::CalculateCommon(int f, bool all, int l) {
    string msg = "ImprovedTrajectory::CalculateCommon("+ToStr(f)+","+ToStr(all)+","+
      ToStr(l)+") : ";
    ...
    cox::tictac::func tt(tics, "ImprovedTrajectory::CalculateCommon");

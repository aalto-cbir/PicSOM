// -*- C++ -*- 	$Id: ExtBatch.h,v 1.4 2020/03/30 08:57:24 jormal Exp $
/**
   \file ExtBatch.h

   \brief Declarations and definitions of class ExtBatch
   
   ExtBatch.h contains declarations and definitions of the class
   ExtBatch, which is a class that calculates the mel-cepstrum feature
   using an external command

   \author Mats Sjoberg <mats.sjoberg@hut.fi>
   $Revision: 1.4 $
   $Date: 2020/03/30 08:57:24 $
...
namespace picsom {
  /// A class that ...
  class ExtBatch : public External {
  public:
    ...
    */
    ExtBatch(const string& img = "", const list<string>& opt = (list<string>)0) {
	...
	\param opt list of options to initialise to
    */
    ExtBatch(const string& img, const string& seg,
    ...
    ExtBatch(bool) : External(false) {}

    virtual Feature *Create(const string& s) const {
      return (new ExtBatch())->SetModel(s);
    }
    ...
    virtual string ShortText() const {
      return "extbatch for any externally called feature extractor.";
    }
  ...
  protected:
    ...
    /// CWD directory to be used
    string dir;

    /// Python's virtualenv to be used
    string venv;
    
    /// Actual command
    vector<string> cmd;
    
    /// Addition to front of LD_LIBRARY_PATH
    string llp;
    ...
    /// Temporary directory. if set, it well be destryed.
    string tmpdir;
  };
}
#endif

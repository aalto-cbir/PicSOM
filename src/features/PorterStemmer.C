// -*- C++ -*- $Id: PorterStemmer.C,v 1.5 2016/10/25 08:13:00 jorma Exp $
   ...
   {  case 'a': if (ends(z, "\02" "al")) break;
	...
	return;
      ...
      case 'n': if (ends(z, "\03" "ant")) break;
		...
		return;
      case 'o': if (ends(z, "\03" "ion") &&
		    (z->b[z->j] == 's' || z->b[z->j] == 't')) break;
...
// void stemstring(struct stemmer * z, char * str) {
//   while(TRUE) {  
//     int ch = (int) *str;	
//     if (ch == 0) return;
//     if (LETTER(ch)) {  
...
// 	i++;
// 	ch = (int) *++str;
// 	if (!LETTER(ch)) { --str; break; }
//       }
//       //printf("calling stem with i=%d\n", i);
//       s[stem(z, s, i - 1) + 1] = 0;
//       /* the previous line calls the stemmer and uses its result to
// 	 zero-terminate the string in s */
//       printf("%s",s);

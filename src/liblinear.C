...
static void transpose(const problem *prob, svm_node **x_space_ret, problem *prob_col)
{
	int i;
	int l = prob->l;
	int n = prob->n;
...
void cross_validation(const problem *prob, const parameter *param, int nr_fold, int *target, double **prob_estim)
{
	int i;
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		for(j=begin;j<end;j++) {
                  target[perm[j]] = predict_probability(submodel,
                                                        prob->x[perm[j]],
                                                        prob_estimates);
                  /* Hack to return probability estimates! */
                  for(int i=0; i<nr_class; i++)
                    if (prob_estim)
                      prob_estim[perm[j]][i] = prob_estimates[i];
...
int predict_values(const struct model *model_, const struct svm_node *x, double *dec_values)
{
	int idx;
	int n;
	if(model_->bias>=0)
...
int predict(const model *model_, const svm_node *x)
{
...
int predict_probability(const struct model *model_, const struct svm_node *x, double* prob_estimates)
{
...
{
  // this change by markus on 2013-04-25 to enable probability estimates also
  // for SVMs, as suggested in http://www.csie.ntu.edu.tw/~cjlin/liblinear/FAQ.html
  return 1;
